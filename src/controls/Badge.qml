...
QQC2.Control {
    ...
    QtObject {
        ...
        readonly property bool labelVisible: label.text.length > 0

        // Be pill-shaped when not very tall, because it looks great
        readonly property bool pillShaped: !labelVisible || label.lineCount === 1

        // Extra horizontal padding looks better when pill-shaped with icons and long labels
        readonly property int horizontalPadding: pillShaped && labelVisible && label.text.length > 2 ? (padding + Kirigami.Units.smallSpacing) : padding
        readonly property int extraTrailingPadding: pillShaped && icon.visible && labelVisible ? Kirigami.Units.smallSpacing : 0
    }
    ...
    background: Rectangle {
        ...
        radius: internal.pillShaped ? root.implicitHeight / 2 : Kirigami.Units.cornerRadius
    }
}
